...
use crate::low_frac::calculate_prob_low_frac;
...
pub struct SNPFrag {
    ...
    pub low_frac_snps: Vec<usize>,
    // index of candidate low fraction variants
...
impl SNPFrag {
    ...
    pub fn get_low_frac_haplotype_baseqs(&mut self, bam_path: &str, region: &Region, phased_fragments: &HashMap<String, i32>) {
        ...
        while let Some(result) = bam_reader.read(&mut record) {
            ...
            if pos > self.candidate_snps[*self.low_frac_snps.last().unwrap()].pos {
                continue;
            }
            ...
            let mut idx = 0; // index in self.low_frac_snps
            let mut snp_pos = -1; // pre-computed position of candidate SNPs
            let mut alleles; // pre-computed alleles of candidate SNPs
            if pos <= self.candidate_snps[*self.low_frac_snps.first().unwrap()].pos {
                snp_pos = self.candidate_snps[self.low_frac_snps[idx]].pos;
                alleles = self.candidate_snps[self.low_frac_snps[idx]].alleles.clone();
            } else {
                // find the first SNP in the read
                while idx < self.low_frac_snps.len() {
                    if self.candidate_snps[self.low_frac_snps[idx]].pos >= pos {
                        break;
                    }
                    idx += 1;
                }
                assert!(
                    idx < self.low_frac_snps.len(),
                    ...
                    b'M' | b'X' | b'=' => {
                        for _ in 0..cg.len() {
                            if pos_on_ref == snp_pos {
                                let low_frac_cand = &mut self.candidate_snps[self.low_frac_snps[idx]];
                                let base = seq[pos_on_query as usize] as char;
                                let mut baseq = record.qual()[pos_on_query as usize];
                                baseq = if baseq < 30 { baseq } else { 30 };
                                let [allele1, allele2] = low_frac_cand.alleles.clone();
                                let ref_allele = low_frac_cand.reference;
                                if allele1 == ref_allele || allele2 == ref_allele {
                                    if base == allele1 || base == allele2 {
                                        if base == ref_allele {
                                            if assignment == 1 {
                                                low_frac_cand.hap_quals.hap1_ref_baseqs.push(baseq);
                                            } else {
                                                low_frac_cand.hap_quals.hap2_ref_baseqs.push(baseq);
                                            }
                                        } else {
                                            if assignment == 1 {
                                                low_frac_cand.hap_quals.hap1_alt_baseqs.push(baseq);
                                            } else {
                                                low_frac_cand.hap_quals.hap2_alt_baseqs.push(baseq);
                                            }
                                        }
                                    }
                                }
                                idx += 1;
                                if idx < self.low_frac_snps.len() {
                                    snp_pos = self.candidate_snps[self.low_frac_snps[idx]].pos;
                                    alleles = self.candidate_snps[self.low_frac_snps[idx]].alleles.clone();
                                }
                            }
        ...
        // for idx in self.low_frac_snps.iter() {
        //     println!("low_frac snp:{:?}\n, hap1_ref_baseqs:{:?}\n, hap1_alt_baseqs:{:?}\n, hap2_ref_baseqs:{:?}\n, hap2_alt_baseqs:{:?}\n",
    ...
    pub fn eval_low_frac_var_phase(&mut self, min_phase_score: f32, apply_downsampling: bool) {
        for ti in self.low_frac_snps.iter() {
            let snp = &mut self.candidate_snps[*ti];
            if snp.snp_cover_fragments.len() == 0 {
                // no surranding haplotype links
                snp.single = true;
            ...
            if phase_score1.max(phase_score2) >= min_phase_score as f64 {
                ...
                // out-of-phase
                snp.non_selected = true;
                snp.cand_low_frac = true;
                snp.for_phasing = false;
            }
        }
    }
    ...
    pub fn assign_snp_haplotype_genotype(&mut self, apply_downsampling: bool) {
        ...
        for ti in 0..self.candidate_snps.len() {
            ...
            //     if ref_allele_cnt + alt_allele_cnt > 0 && alt_allele_cnt as u32 >= low_frac_allele_cnt_cutoff && alt_allele_cnt as f32 / (ref_allele_cnt + alt_allele_cnt) as f32 >= low_frac_allele_frac_cutoff {
            //         // calculate low_frac mutation probability
            //         let (hap1_allele_class, hap2_allele_class) = calculate_prob_low_frac(&snp.hap_quals.hap1_ref_baseqs, &snp.hap_quals.hap1_alt_baseqs, &snp.hap_quals.hap2_ref_baseqs, &snp.hap_quals.hap2_alt_baseqs, 0.3);
            //         if hap1_allele_class.allcls == 0 && hap2_allele_class.allcls == 2 {
            //             let low_frac_score = -10.0_f64 * (1.0 - hap2_allele_class.prob).log10();
            //             snp.cand_low_frac = true;
            //             snp.low_frac = true;
            //             snp.variant_type = 1;
            //             snp.low_frac_score = low_frac_score;
            //             snp.phase_score = 0.0;
            //             // println!("low_frac snp:{}, score: {}", snp.pos, low_frac_score);
    ...
    pub fn assign_reads_haplotype(&mut self, read_assignment_cutoff: f64, apply_downsampling: bool) -> HashMap<String, i32> {
        ...
        for k in 0..self.fragments.len() {
            ...
            let mut ps: Vec<i32> = Vec::new();
            let mut probs: Vec<f64> = Vec::new();
            for fe in self.fragments[k].list.iter_mut() {
                // if fe.phase_site == false { continue; }
                if fe.phase_site == false && self.candidate_snps[fe.snp_idx].for_phasing {fe.phase_site=true;}
    ...
    pub fn detect_low_frac_by_het(&mut self, bam_path: &str, region: &Region) {
        if self.low_frac_snps.len() == 0 {
            return;
        }
        // detect confident low_frac mutation with phasing result of high allele fraction het snps.
        // 1. assign phased result for candidate low_frac sites.
